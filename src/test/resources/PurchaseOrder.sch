...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:s="http://test.se" queryBinding="xpath2">
  ...
  <pattern>
    <rule context="shipTo | shipDate | Items">
      ...
      <assert test="parent::*/@orderDate">A purchase order should have an orderDate attribute.</assert>
    </rule>
    <rule context="/shipTo | /shipDate | /Items">
      <report test="self::*">
        The element
  ...
  <pattern>
    <rule context="PurchaseOrder">
      <assert test="shipTo">A purchase order element should have a shipping address. There should be a shipTo element contained by the PurchaseOrder element.</assert>
      <assert test="@orderDate">A purchase order element should have a shipping address. There should be a shipTo element contained by the PurchaseOrder element.</assert>
      <assert test="@orderDate1">A purchase order element should have a shipping address. There should be a shipTo element contained by the PurchaseOrder element.</assert>
      <assert test="1 eq 2">testar lite</assert>
    </rule>
    <rule context="shipTo">
      <assert test="name">A shipping address element should have a name element.</assert>
    </rule>
    <rule context="Items">
    ...
    <rule context="comment">
      ...
      <assert test="parent::Item | parent::PurchaseOrder ">A comment can only appear in an Item or a Purchase Order</assert>
    </rule>
  </pattern>
</schema>